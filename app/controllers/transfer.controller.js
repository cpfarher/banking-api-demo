...
exports.create = (req, res) => {
  ...
  Promise.all([Account.findByPk(fromAccountId), Account.findByPk(toAccountId)])
    .then(([fromAccount, toAccount]) => {
      const existsAccounts=fromAccount && toAccount
      const notEqualAccount = fromAccount.id != toAccount.id
      const validAmount = Math.abs(amount) > 0
      const outgoingAccountHasBalance = (fromAccount.balance - Math.abs(amount)) >= 0

      if (
        existsAccounts &&
        notEqualAccount &&
        validAmount &&
        outgoingAccountHasBalance
      ) {
        // register transfer:
        const transfer = {
          // ip: req.body.ip,
        ...
        // check balance amount of outgoing Account
        Transfer.create(transfer).then((data) => {
        ...
        if (!existsAccounts) {
          res.status(422).send({
            message: "fromAccountId or toAccountId does must exists",
          });
        } else if (!notEqualAccount) {
          res.status(422).send({
        ...
        } else if (!outgoingAccountHasBalance) {
          res.status(422).send({
            message: "Account does not have enough balance to transfer",
          });
        } 
        return;
      }
    ...
    .catch((err) => {
      res.status(500).send({
        message: err.message || "There are some problem creating Transfer",
      });
    });
};
